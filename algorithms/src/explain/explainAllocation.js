...
export function explainAllocation({
  ...
  const suburbContext = suburb
    ? { suburb, isCouncilPrioritySuburb: isPrioritySuburb(suburb), seifa: resolveAreaDisadvantage(suburb) }
    : null;
  ...
  if (suburbContext?.seifa && suburbContext.seifa.source !== 'LGA-wide default') {
    summaryParts.push(
      `${suburbContext.suburb} sits at national SEIFA disadvantage percentile ${suburbContext.seifa.percentile} (ABS Census 2021, via ${suburbContext.seifa.source}) — more disadvantaged than ${100 - suburbContext.seifa.percentile}% of Australian suburbs.`
    );
  }
