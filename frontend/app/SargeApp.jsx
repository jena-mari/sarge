...
const consentItems = [
 ...
 ['export_data_consent','Contributor agrees SARGE can use verified export data to calculate civic recognition credits. SARGE does not buy the electricity or change retailer payments.'],
];
...
function EnergyFlow({ compact = false }) {
  const steps = [
    ...
    ['Counted contribution',85,'Verified exports recognised by SARGE','contributed'],
  ];
...
function ReportGenerator({ inline = false }) {
  ...
  const download=()=>{const rows=['Metric,Value','Solar generated,160 kWh','Property consumption,74 kWh','Spare energy,86 kWh','Verified export contribution,85 kWh','Sarge Credits created,85','Support allocations associated,4']; const blob=new Blob([rows.join('\n')],{type:'text/csv'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='sarge-energy-report.csv'; a.click(); URL.revokeObjectURL(a.href)};
  return <section className={`report-card ${inline?'report-card--inline':''}`}><div className="report-icon"><FileText/></div><p className="eyebrow">Reports</p><h2>Your Energy Report</h2><p>See your verified exports, recognition credits and recorded program impact.</p><div className="segmented" aria-label="Report period">{['This month','Last 3 months','Last 6 months','Custom'].map(x=><button key={x} className={period===x?'active':''} onClick={()=>{setPeriod(x);setGenerated(false)}}>{x}</button>)}</div>{generated?<div className="report-preview"><div><span>Generated</span><strong>160 kWh</strong></div><div><span>Used</span><strong>74 kWh</strong></div><div><span>Spare</span><strong>86 kWh</strong></div><div><span>Verified contribution</span><strong>85 kWh</strong></div><div><span>Credits created</span><strong>85</strong></div><div><span>Support allocations associated</span><strong>4</strong></div></div>:<div className="report-placeholder"><span>{period}</span><div className="report-line"/><div className="report-line short"/><p>Your report will turn your energy data into a simple, downloadable summary.</p></div>}<Button onClick={()=>generated?download():setGenerated(true)}>{generated?<><Download size={18}/> Download report</>:<>Generate report <ArrowRight size={18}/></>}</Button></section>;
}
...
function Contribute() {
 ...
 return <Shell title="Recognise your exported solar" eyebrow="Contribution"><div className="contribute-layout"><section><div className="today-flow"><div><span>Verified export today</span><strong>{formatKwh(verifiedExportToday)} kWh</strong></div><ArrowRight/><div><span>Weekly contribution cap</span><strong>{formatKwh(weeklyDonationCap)} kWh</strong></div><ArrowRight/><div className="pink"><span>Today’s SARGE Credits</span><strong>{formatKwh(creditsToday)}</strong></div></div>{homeBattery==='Yes'&&<label className={`choice reserve-choice ${reserve?'selected':''}`}><input type="checkbox" checked={reserve} onChange={e=>setReserve(e.target.checked)}/><span>Only count exports after the battery reserve is met</span>{reserve&&<Check/>}</label>}</section><section className="pledge-card"><h2>Weekly contribution cap</h2><div className="slider-wrap"><label htmlFor="weekly-cap">Maximum verified export counted this week</label><input id="weekly-cap" type="range" min="0" max={estimatedWeeklyExport} step="1" value={weeklyDonationCap} onChange={e=>updateWeeklyCap(e.target.value)}/><div className="number-field"><input aria-label="Weekly contribution cap" type="number" min="0" max={estimatedWeeklyExport} value={weeklyDonationCap} onChange={e=>updateWeeklyCap(e.target.value)}/><span>kWh</span></div></div><div className="calculation"><span>Estimated weekly export</span><strong>{formatKwh(estimatedWeeklyExport)} kWh</strong><span>Verified export today</span><strong>{formatKwh(verifiedExportToday)} kWh</strong><span>Weekly contribution cap</span><strong>{formatKwh(weeklyDonationCap)} kWh</strong><span>Today’s SARGE Credits</span><strong>{formatKwh(creditsToday)}</strong><span>Weekly cap remaining after today</span><strong>{formatKwh(weeklyCapRemainingAfterToday)} kWh</strong></div><Button onClick={confirm} disabled={creditsToday<=0}>Review contribution <ArrowRight size={18}/></Button></section></div></Shell>;
}
...
function ConfirmDonationSummary({ propertyLabel, verifiedBy, verifiedExportToday, weeklyDonationCap, creditsCreated, onConfirm, showButton = false }) {
 return <><div className="confirm-donation-grid"><article><span>Property</span><strong>{propertyLabel}</strong></article><article><span>Weekly contribution cap</span><strong>{formatKwh(weeklyDonationCap)} kWh / week</strong></article><article className="confirm-donation-conversion"><span>Recognition credits created</span><div className="confirm-donation-equation"><div><strong>{formatKwh(verifiedExportToday)} kWh</strong><small>verified export</small></div><b>=</b><div><strong>{formatKwh(creditsCreated)}</strong><small>SARGE Credits</small></div></div></article><article className="confirm-donation-verified"><span>Verification method</span><strong>{verifiedBy}</strong></article></div><article className="confirm-donation-info"><h2>Civic reward rates</h2><div className="civic-rate-grid">{civicRewardRates.map(item=><div className="civic-rate-tile" key={item.credits}><strong>{item.credits}</strong><b>=</b><small>{item.reward}</small></div>)}</div></article><p className="confirm-donation-reassurance">You can change or stop your contribution pledge anytime.</p><p className="confirm-donation-reassurance">SARGE Credits are civic reward points, not payment for electricity. SARGE does not change your retailer account; any feed-in tariff remains governed by your retailer plan.</p>{showButton&&<Button onClick={onConfirm}>Confirm contribution <ArrowRight size={18}/></Button>}</>;
}
...
function ConfirmDonation() {
 ...
 const confirm=()=>{const contributedKwh=Number(contribution.contributed_kwh ?? creditsCreated);const confirmedContribution={...contribution,donor_id:contribution.donor_id || 'D001',pledged_kwh:Number(contribution.pledged_kwh ?? weeklyDonationCap),verified_spare_kwh:Number(contribution.verified_spare_kwh ?? verifiedExportToday),contributed_kwh:contributedKwh,sarge_credits_created:Number(contribution.sarge_credits_created ?? contributedKwh),credits_created_today:Number(contribution.credits_created_today ?? contributedKwh),confirmed:true,confirmed_at:new Date().toISOString(),allocation_status:'ready'};localStorage.setItem('sargeContribution',JSON.stringify(confirmedContribution));localStorage.setItem('sargeOnboardingComplete','true');localStorage.setItem('sargeJustConfirmedCredits',JSON.stringify(creditsCreated));nav('/overview')};
 return <Shell title="Confirm Contribution" eyebrow="Confirm Contribution"><section className="confirm-donation-card"><ConfirmDonationSummary propertyLabel={`${contribution.property_type || draft.property || 'Home'} in ${contribution.suburb || draft.suburb || 'Dapto'}`} verifiedBy={formatVerificationMethod(contribution.verification_method)} verifiedExportToday={verifiedExportToday} weeklyDonationCap={weeklyDonationCap} creditsCreated={creditsCreated} onConfirm={confirm} showButton/></section></Shell>;
}
 ...
 return <><Navbar/><main className="page rewards-page">
  ...
  <section className="rewards-section credit-balance-section" aria-labelledby="credit-balance-title">
   <div>
    ...
    <p>1 SARGE Credit = 1 verified exported kWh counted toward your contribution.</p>
    <p>SARGE Credits are civic reward points, not payment for electricity. Your retailer account and feed-in tariff remain separate.</p>
   </div>
   <dl>
    <div>
...
function Reports() { const totals=energyHistory.reduce((a,x)=>({generated:a.generated+x.generated,used:a.used+x.used,spare:a.spare+x.spare,contributed:a.contributed+x.contributed}),{generated:0,used:0,spare:0,contributed:0}); return <Shell title="Your energy, clearly explained." eyebrow="Reports"><div className="reports-toolbar"><div><h2>Six-month report</h2><p>March–August 2026</p></div><div className="segmented">{['Monthly','Quarterly','Six months','Custom'].map((x,i)=><button className={i===2?'active':''} key={x}>{x}</button>)}</div><Button><Download size={18}/> Download report</Button></div><section className="chart-card"><h2>Generated, used and verified</h2><p className="chart-summary">Across six months, Bella generated {totals.generated} kWh, used {totals.used} kWh and counted {totals.contributed} kWh as verified contributions.</p><div className="chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={energyHistory} margin={{top:20,right:10,left:-15,bottom:0}}><CartesianGrid vertical={false} stroke="#d9d9d9"/><XAxis dataKey="month" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip cursor={{fill:'#f5f5f5'}}/><Legend/><Bar dataKey="generated" name="Generated" fill="#111" radius={[6,6,0,0]}/><Bar dataKey="used" name="Used" fill="#7D69E8" radius={[6,6,0,0]}/><Bar dataKey="contributed" name="Verified contribution" fill="#F893F6" stroke="#111" strokeWidth={1} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer></div></section><h2 className="impact-title">Your six-month impact</h2><div className="impact-grid">{[['Total solar generated',totals.generated+' kWh'],['Total energy used',totals.used+' kWh'],['Total spare energy',totals.spare+' kWh'],['Total verified contribution',totals.contributed+' kWh'],['SARGE Credits created',totals.contributed],['Households supported','18']].map(([l,v])=><div key={l}><span>{l}</span><strong>{v}</strong></div>)}</div></Shell> }
...
function Onboarding() {
 ...
 if(screen==='pledge') return <main className="property-step pledge-step"><header className="property-step__nav"><div className="brand-lockup" aria-label="Sarge, Wollongong Renewable Energy Ecosystem"><img src={heroAssets.mark} alt="" className="brand-lockup__mark"/><strong className="brand-lockup__name">Sarge</strong><img src={heroAssets.divider} alt="" className="brand-lockup__divider"/><span className="brand-lockup__tag">Wollongong Renewable<br/>Energy Ecosystem</span></div><a className="button property-step__contact" href="mailto:hello@sarge.community">Contact Us</a></header><StepDots active={4}/><section className="property-step__card pledge-step__card"><div className="property-step__top"><button className="property-step__back" type="button" onClick={()=>setScreen('data-source')}><ChevronLeft size={22}/> Back</button><button className="property-step__top-continue pledge-step__continue" type="button" onClick={continueToConfirmDonation}>Review contribution <ArrowRight size={22}/></button></div><h1 className="data-source-step__title">Set your contribution pledge</h1><div className="pledge-step__layout"><section className="pledge-step__metrics"><article className="pledge-step__credits pledge-step__conversion"><span>Verified export</span><strong>{formatKwh(verifiedExportToday)} kWh = {formatKwh(creditsToday)} SARGE Credits</strong></article><article><span>Weekly contribution cap</span><strong>{formatKwh(weeklyDonationCap)} kWh</strong></article><article><span>Weekly cap remaining</span><strong>{formatKwh(weeklyCapRemainingAfterToday)} kWh</strong></article></section><section className="pledge-step__control"><label htmlFor="onboarding-weekly-cap">Weekly contribution cap</label><div className="pledge-step__estimate"><span>Estimated weekly spare solar</span><strong>{formatKwh(estimatedWeeklyExport)} kWh</strong></div><input id="onboarding-weekly-cap" type="range" min="0" max={estimatedWeeklyExport} step="1" value={weeklyDonationCap} onChange={e=>updateWeeklyCap(e.target.value)}/><div className="pledge-step__number"><input aria-label="Weekly contribution cap" type="number" min="0" max={estimatedWeeklyExport} value={weeklyDonationCap} onChange={e=>updateWeeklyCap(e.target.value)}/><span>kWh</span></div></section></div></section></main>;
 if(screen==='confirm-donation') return <main className="property-step confirm-donation-step"><header className="property-step__nav"><div className="brand-lockup" aria-label="Sarge, Wollongong Renewable Energy Ecosystem"><img src={heroAssets.mark} alt="" className="brand-lockup__mark"/><strong className="brand-lockup__name">Sarge</strong><img src={heroAssets.divider} alt="" className="brand-lockup__divider"/><span className="brand-lockup__tag">Wollongong Renewable<br/>Energy Ecosystem</span></div><a className="button property-step__contact" href="mailto:hello@sarge.community">Contact Us</a></header><StepDots active={5}/><section className="property-step__card confirm-donation-step__card"><div className="property-step__top"><button className="property-step__back" type="button" onClick={()=>setScreen('pledge')}><ChevronLeft size={22}/> Back</button></div><h1 className="data-source-step__title">Confirm Contribution</h1><ConfirmDonationSummary propertyLabel={`${property} in ${suburb}`} verifiedBy={formatVerificationMethod(method)} verifiedExportToday={verifiedExportToday} weeklyDonationCap={weeklyDonationCap} creditsCreated={creditsToday} onConfirm={finishOnboarding} showButton/></section></main>;
 return <main className="landing-hero"><video className="landing-hero__background" autoPlay muted loop playsInline preload="metadata" aria-hidden="true"><source src="/assets/figma/Sarge.mp4" type="video/mp4"/></video><div className="landing-hero__video-shade" aria-hidden="true"/><header className="landing-hero__nav"><div className="brand-lockup" aria-label="Sarge, Wollongong Renewable Energy Ecosystem"><img src={heroAssets.mark} alt="" className="brand-lockup__mark"/><strong className="brand-lockup__name">Sarge</strong><img src={heroAssets.divider} alt="" className="brand-lockup__divider"/><span className="brand-lockup__tag">Wollongong Renewable<br/>Energy Ecosystem</span></div><a className="button button--contact" href="mailto:hello@sarge.community">Contact Us</a></header><video className="landing-hero__illustration landing-hero__gong-video" autoPlay muted loop playsInline preload="metadata" aria-label="A moving view of Wollongong"><source src="/assets/figma/wgong.mp4" type="video/mp4"/></video><section className="landing-hero__content"><h1><span>Share the sun,</span><span>with <em>The Gong.</em></span></h1><p><strong>Sarge</strong> recognises verified solar exports with civic reward points and uses contribution records to support fair local energy-hardship programs.</p><div className="landing-hero__actions"><button className="button button--hero" type="button" onClick={()=>setScreen('property')}>Get Started <ArrowRight size={26}/></button><a className="button button--mobile-contact" href="mailto:hello@sarge.community">Contact Us</a></div></section></main>;
}
