...
import { ArrowRight, Check, Download, FileText, Menu, Users, X, Zap } from 'lucide-react';
...
function Onboarding() {
 ...
 const enterApp=()=>{localStorage.setItem('sargeOnboardingComplete','true');nav('/overview')};
 return <main className="landing-hero"><img className="landing-hero__background" src={heroAssets.background} alt="" aria-hidden="true"/><header className="landing-hero__nav"><div className="brand-lockup" aria-label="Sarge, Wollongong Renewable Energy Ecosystem"><img src={heroAssets.mark} alt="" className="brand-lockup__mark"/><strong className="brand-lockup__name">Sarge</strong><img src={heroAssets.divider} alt="" className="brand-lockup__divider"/><span className="brand-lockup__tag">Wollongong Renewable<br/>Energy Ecosystem</span></div><a className="button button--contact" href="mailto:hello@sarge.community">Contact Us</a></header><img className="landing-hero__illustration" src={heroAssets.family} alt="" aria-hidden="true"/><section className="landing-hero__content"><h1>Share the sun,<br/>with <em>The Gong.</em></h1><p>Sarge lets you contribute solar energy you don’t use to support Wollongong households experiencing energy hardship — while recognising you for helping.</p><div className="landing-hero__actions"><button className="button button--hero" type="button" onClick={enterApp}>Get Started <ArrowRight size={26}/></button><a className="button button--mobile-contact" href="mailto:hello@sarge.community">Contact Us</a></div></section></main>;
}
